...
fx.Base.prototype = {
	...
	custom: function(from, to) {
		if (this.timer != null)
		{
			if(this.to < this.max) // decrease -> increase
			{
				this.duration = this.options.duration*(1-this.now/this.max);
				this.to = this.max;
			}
			else // increase -> decrease
			{
				this.duration = this.options.duration*(this.now/this.max);
				this.to = this.min;
			}
			this.from = this.now;
			var time = (new Date).getTime();
...
fx.Opacity.prototype = Object.extend(new fx.Base(), {
	...
	increase: function() {
		//if (this.now == 1) this.now = 0.9999; performance probleme?
